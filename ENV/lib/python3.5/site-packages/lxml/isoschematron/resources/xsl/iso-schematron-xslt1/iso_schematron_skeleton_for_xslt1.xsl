...
<!--
 ...
 Conventions: Meta-stylesheets that override this may use the following parameters
    ...
    diagnose= yes | no    Add the diagnostics to the assertion tests in reports
...
<!--
  ...
  VERSION INFORMATION 
   ...
   Version: 2008-07-14 minor update for inclusion experiments
   	* RJ Clean up zero-length fragment tests on include
   	* RJ Add experimental support for include containers 
   	* RJ For path generation, tests for //iso:schema not just /iso:schema, for potential embedded Schematron support
   ...
   Version: 2007-04-04
   	* RJ debug mode param
	* RJ alter mixed tests to only tests mixed branches, so the same document
...
<!--
			<axsl:choose>
				<axsl:when tests="count(. | ../namespace::*) = count(../namespace::*)">
...
<xsl:template name="handle-namespace">
       ...
       <xsl:choose>
        ...
        <!-- Not tested yet       
    	<xsl:when tests="function-available('msxsl:node-set')">
      		<xsl:variable name="ns-dummy-elements">
...
<!-- ============================================================== -->
	...
	<!-- Swallow old namespace elements: there is an upfront tests for them elsewhere -->
